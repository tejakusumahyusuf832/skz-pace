...
--                         like_to_view_ratio, margin_like_to_view_ratio, daily_view_velocity
...
SELECT
    ...
    -- --- Gets Column daily_view_velocity ---
    (
        MAX(view_count) FILTER (WHERE exit_rank = 1) -
        MAX(view_count) FILTER (WHERE entry_rank = 1)
    )::REAL / (
